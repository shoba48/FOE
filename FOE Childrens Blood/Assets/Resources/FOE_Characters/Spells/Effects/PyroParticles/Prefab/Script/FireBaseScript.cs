    ...
    public class FireBaseScript : MonoBehaviour
    {
        ...
        public static void CreateExplosion(Vector3 pos, float radius, float force)
        {
            ...
            foreach (Collider h in objects)
            {
                ...
                // For My Game
                Attributes attr = h.GetComponent<Attributes>();
                if (r != null)
                {
                    //r.AddExplosionForce(force, pos, radius); getällt mir nicht
                    Vector3 direction = r.transform.position - pos;
                    r.velocity += (direction * force);
                }
                if (attr != null)
                {
                    float distance = Vector3.Distance(attr.transform.position, pos);
                    attr.AddLifePoints(Mathf.RoundToInt(-force / (1 + distance)));
                }
            }
        }
